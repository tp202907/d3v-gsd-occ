...
def test2(window):
    ...
    main_widget = InteractiveTableChartWidget(window)
    data = [[1, 95, 15.2, 4.0, 469.25, 7.77, 0.502],
            [2, 95, 16.0, 4.2, 637.97, 9.05, 0.532],
            [3, 95, 16.8, 4.4, 701.06, 8.83, 0.544],
            [4, 100, 15.2, 4.0, 448.09, 7.53, 0.502],
            [5, 100, 16.0, 4.2, 601.18, 9.06, 0.532],
            [6, 100, 16.8, 4.4, 609.04, 8.56, 0.544],
            [7, 105, 15.2, 4.0, 479.05, 8.12, 0.502],
            [8, 105, 16.0, 4.2, 566.46, 9.06, 0.532],
            [9, 105, 16.8, 4.4, 653.69, 9.25, 0.544]]
    for row in data:
        row.append(3254.9/(row[1]*row[2]*row[3]))
    data_names = ['No.', 'L, m', 'B,m', 'T,m', 'R, kN', 'KM, m', 'Cp']
    n_cols = len(data_names)
    data_formats = ['{:d}', '{:.1f}', '{:.1f}', '{:.1f}', '{:.2f}', '{:.2f}', '{:.3f}', '{:.3f}']
    chart_data_pairs = [4, 5]
    n_rows = len(data)
    main_widget = InteractiveTableChartWidget(window)
    main_widget.set_data(n_cols, n_rows, data_names, data_formats, data, chart_data_pairs, 'scatter')
    window.setCentralWidget(main_widget)
